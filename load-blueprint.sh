...
cluster_json_hosts=$(ruby ${bin}/extract_hosts.rb ${bin}/${blueprint_name}/cluster.json)
exit_code=$?
...
if [[ ${exit_code} -ne 0 ]]; then
  ...
  while read cluster_json_line; do
    ...
    if [[ $? -ne 0 ]]; then
      echo "Failed."
      fail "'${cluster_json_line}' is not reachable by ping, is ${bin}/${blueprint_name}/cluster.json correct?"
    fi
    echo "Good!"
